...
namespace TareasMVC.Controllers
{
    public class UsuariosController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpGet]
        public ChallengeResult LoginExterno(string proveedor, string urlRetorno = null) // Significa que se va a redirigir al usuario a una fuente externa donde se puede loguear
        {
            var urlRedireccion = Url.Action("RegistrarUsuarioExterno", values: new { urlRetorno }); // RegistrarUsuarioExterno: accion que recibe la data del usuario
            var propiedades = signInManager.ConfigureExternalAuthenticationProperties(proveedor, urlRedireccion);
            return new ChallengeResult(proveedor, propiedades);
        }

        [AllowAnonymous] 
        public async Task<IActionResult> RegistrarUsuarioExterno(string urlRetorno = null, string remoteError = null) // Recibe la data del proveedor de identidad. remoteError: Errores que retorna el proveedor de identidad
        {
            urlRetorno = urlRetorno ?? Url.Content("~/"); // Si es nulo
            ...
            if(remoteError is not null)
            {
                ...
                return RedirectToAction("login", routeValues: new { mensaje }); // Redirijo al login y le paso el mensaje
            }

            var info = await signInManager.GetExternalLoginInfoAsync(); // Obtiene la informacion del login externo
            if(info == null)
            {
            ...
            if(info.Principal.HasClaim(c => c.Type == ClaimTypes.Email)) // Si no existe el usuario, lo registramos
            {
